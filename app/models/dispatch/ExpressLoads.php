...
class ExpressLoads
{
	...
	function express_loads_document_update_roc_file($param,$param_file){
		$status=false;
		$message=null;
		$response=null;
		...
		if(isset($param['express_load_eid'])){
			$express_load_id=$Enc->safeurlde($param['express_load_eid']);
			$get_driver=mysqli_query($GLOBALS['con'],"SELECT `express_load_id` FROM `d_express_loads` WHERE `express_load_id`='$express_load_id'");
...
//----verify document details to be uploaded
		...
		if($dataValidation){
			...
			//--upload image 	
			$send_param=array(
				'path'=>'documents/loads'
			);
			$new_file_name=$express_load_id.'-ROC-'.date('ymdHi').'.'.$fileType;
...
*/
			...
			$update_po_info=mysqli_query($GLOBALS['con'],"UPDATE `d_express_loads` SET `express_load_roc_file`='$new_file_name',`express_load_roc_file_remarks`='$remarks',`express_load_roc_updated_on`='$time',`express_load_roc_updated_by`='".USER_ID."' WHERE `express_load_id`='$express_load_id'");
			if(!$update_po_info){
	...
	function load_status_wise_total($param){
		...
		$qEx=mysqli_query($GLOBALS['con'],"SELECT `load_status_id`,`load_status_name`, COUNT(`express_load_id`) AS `total` FROM `d_load_status` LEFT JOIN `d_express_loads` ON `d_load_status`.`load_status_id`=`d_express_loads`.`express_load_status_id_fk` WHERE `express_load_id_status`='ACT' GROUP BY `load_status_id` ORDER BY `load_status_name`");
...
//-----Apply fitlers ends
		...
		while ($rows=mysqli_fetch_assoc($qEx)) {
			...
			$el_list=[
				...
				'roc_file'=>($rows['express_load_roc_file']!="")?DOCUMENTS_ROOT.'documents/loads/'.$rows['express_load_roc_file']:'',
		...
		if($dataValidation){
			...
			if(mysqli_num_rows($q)==1){
				...
				$response['details']=[
					...
					'rate'=>$rows['eld_rate'],
					'status_id'=>$rows['express_load_status_id_fk'],
					'booked_by_id'=>$booked_by_user['user_id'],
					'booked_by_code'=>$booked_by_user['user_code'],
					'is_team_driver'=>($rows['express_load_is_team_driver']==1)?true:false,
					'alloted_driver_id'=>($rows['driver_a_id']!=null)?$rows['driver_a_id']:'',
					'alloted_driver_code'=>($rows['driver_a_code']!=null)?$rows['driver_a_code']:'',
					'alloted_driver_name'=>$driver_name,
