	...
	function trips_update($param){
		...
		if(in_array('P0121', USER_PRIV)){
			...
			if(isset($param['update_eid'])){
				...
				$get_all_paid_payments=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'],"SELECT COUNT(`payment_id`) AS `total_paids` FROM `driver_payments` WHERE `payment_status`='ACT' AND `payement_category`='TRIP-EARNINGS' AND `payment_trip_id_fk`='$update_id' AND `payment_pay_status`='PAID'"));
...
//-------Delete old saved Stops,Salary paramters, drivers
					mysqli_query($GLOBALS['con'],"DELETE FROM `driver_payments` WHERE `payement_category`='TRIP-EARNINGS' AND`payment_pay_status`='UNPAID' AND `payment_trip_id_fk`='$update_id'");
					mysqli_query($GLOBALS['con'],"DELETE FROM `trip_drivers` WHERE `trip_driver_trip_id_fk`='$update_id'");
					mysqli_query($GLOBALS['con'],"DELETE FROM `trip_stops` WHERE `trip_stop_trip_id`='$update_id'");
					...
					foreach ($drivers_array as $dA) {
						...
						$salary_parameters=$this->driver_salary_parameter(array('trip_id' =>$update_id ,'driver_id'=>$dA['id'] ));
...
function trips_list($param){
	...
	$q="SELECT `trip_id`,`truck_id`, `truck_code`,`group_name`, `trip_total_miles`,`trip_ppm`,`trip_incentive_per_mile`,`trip_incentive`, `trip_start_date`, `trip_end_date`,`trip_approval_status_id_fk`,`trip_added_on`,`trip_added_by`,`trip_approved_on`,`trip_approved_by`,(SELECT SUM(ROUND(`trip_salary_parameter_amount`,2)) FROM `trip_salary_parameters` LEFT JOIN `salary_parameters` ON `salary_parameters`.`parameter_id`=`trip_salary_parameters`.`trip_salary_parameter_parameter_id` WHERE `trip_salary_parameter_status`='ACT' AND `trip_salary_parameter_trip_id_fk`=`trip_id` AND `parameter_type_id_fk`='REIMBURSEMENT') AS `trip_salary_parameters_reimbursement` ,(SELECT SUM(ROUND(`trip_salary_parameter_amount`,2)) FROM `trip_salary_parameters` LEFT JOIN `salary_parameters` ON `salary_parameters`.`parameter_id`=`trip_salary_parameters`.`trip_salary_parameter_parameter_id` WHERE `trip_salary_parameter_status`='ACT' AND `trip_salary_parameter_trip_id_fk`=`trip_id` AND `parameter_type_id_fk`='EARNING') AS `trip_salary_parameters_earning`,(SELECT SUM(ROUND(`trip_salary_parameter_amount`,2)) FROM `trip_salary_parameters` LEFT JOIN `salary_parameters` ON `salary_parameters`.`parameter_id`=`trip_salary_parameters`.`trip_salary_parameter_parameter_id` WHERE `trip_salary_parameter_status`='ACT' AND `trip_salary_parameter_trip_id_fk`=`trip_id` AND `parameter_type_id_fk`='DEDUCTION') AS `trip_salary_parameters_deduction`,(SELECT COUNT(`payment_id`) FROM `driver_payments` WHERE `payment_status`='ACT' AND `payement_category`='TRIP-EARNINGS' AND `payment_trip_id_fk`=`trip_id` AND `payment_pay_status`='PAID') AS `total_paid_payment_for_trip` FROM `trips` LEFT JOIN `trucks` ON `trucks`.`truck_id`=`trips`.`trip_truck_id_fk` LEFT JOIN `driver_groups` ON `driver_groups`.`group_id`=`trips`.`trip_driver_group_id_fk`  WHERE `trip_status`='ACT'";
...
//----Apply Filters starts
	...
	if(isset($param['driver_id']) && $param['driver_id']!=''){
		...
		$q.=" AND (SELECT COUNT(`trip_driver_id`) FROM `trip_drivers` WHERE `trip_driver_trip_id_fk`=`trip_id` AND `trip_driver_driver_id_fk`='$driver_id')>0";
	}
//
...
//---fetch trip_drivers records
		$fetch_drivers=mysqli_query($GLOBALS['con'],"SELECT `driver_code`,`driver_id`, `driver_name_first` FROM `trip_drivers`  LEFT JOIN `drivers` ON `drivers`.`driver_id`=`trip_drivers`.`trip_driver_driver_id_fk`  WHERE `trip_driver_status`='ACT' AND `trip_driver_trip_id_fk`='".$rows['trip_id']."'");
		$trip_drivers_array=[];

		$is_filtered_driver_exists=false;
		while ($td=mysqli_fetch_assoc($fetch_drivers)) {
			$tdr=[];
			$tdr['driver_eid']=$Enc->safeurlen($td['driver_code']);
			$tdr['driver_code']=$td['driver_code'];
			$tdr['driver_name']=$td['driver_name_first'];
			array_push($trip_drivers_array, $tdr);
		...
		if(isset($param['approve_eid_list'])){
			...
			if($list_valid){
				...
				foreach ($approve_eid_list as $al) {
					...
					$get_drivers_earnings=mysqli_query($GLOBALS['con'],"SELECT `trip_driver_id`,`trip_driver_driver_id_fk`, `trip_driver_net_earnings` FROM `trip_drivers` WHERE `trip_driver_trip_id_fk`='$approve_id'");
					...
					while ($gde=mysqli_fetch_assoc($get_drivers_earnings)) {
						...
						$make_statment=mysqli_query($GLOBALS['con'],"INSERT INTO `driver_payments`(`payment_id`, `payment_driver_id_fk`, `payement_category`, `payment_type`,`payment_amount` ,`payment_pay_status`, `payment_added_on`, `payment_added_by`, `payment_trip_id_fk`, `payment_trip_driver_id_fk`,`payment_status`) VALUES ('$new_driver_payment_id','".$gde['trip_driver_driver_id_fk']."','TRIP-EARNINGS','CR','".$gde['trip_driver_net_earnings']."','UNPAID','$time','$USERID','$approve_id','".$gde['trip_driver_id']."','ACT')");
					}
...
function trips_details_for_updation($param){
	...
	if(isset($param['trip_eid'])){
		...
		$Enc=new Enc;
		$trip_id=$Enc->safeurlde($param['trip_eid']);
		$query="SELECT `trip_id`, `trip_truck_id_fk`,`trip_driver_group_id_fk`,`trip_ppm_plan_group_id`,`trip_ppm`,`trip_incentive_per_mile` FROM `trips`  WHERE `trip_status`='ACT' AND `trip_id`='$trip_id'  AND (SELECT COUNT(`payment_id`) FROM `driver_payments` WHERE `payment_status`='ACT' AND `payement_category`='TRIP-EARNINGS' AND `payment_trip_id_fk`=`trip_id` AND `payment_pay_status`='PAID')<1";
		...
		if(mysqli_num_rows($get)==1){
			...
			function salary_parameters_of_driver_for_trip($driver_id,$trip_id){
				$get_parameters_q=mysqli_query($GLOBALS['con'],"SELECT `trip_salary_parameter_parameter_id`, `trip_salary_parameter_amount`  FROM `trip_salary_parameters` WHERE `trip_salary_parameter_trip_id_fk`='$trip_id' AND  `trip_salary_parameter_status`='ACT' AND  `trip_salary_parameter_driver_id_fk`='$driver_id'");
				$parameters_list=[];
...
//---fetch trip_drivers records
			$fetch_drivers=mysqli_query($GLOBALS['con'],"SELECT `trip_driver_id`, `trip_driver_driver_id_fk`,`trip_driver_remarks` FROM `trip_drivers`   WHERE `trip_driver_status`='ACT' AND `trip_driver_trip_id_fk`='$trip_id'");
			$trip_drivers_array=[];
			$drivers=[];
			while ($td=mysqli_fetch_assoc($fetch_drivers)) {
				$driver=[];
				...
				$driver['salary_parameters']=salary_parameters_of_driver_for_trip($td['trip_driver_driver_id_fk'],$rows['trip_id']);
...
////------------------generate new transection id
					...
					$get_driver_payment_id_prefix=date("ymd");
					//---if last trip id is from old month than change the prefix with current month and start counting from 1
					if($get_driver_payment_id_prefix==substr($get_driver_transection_id,0,6)){
						$new_driver_transection_id=$get_driver_payment_id_prefix.sprintf('%04d',(intval(substr($get_driver_transection_id,6))+1));
					}else{
						$new_driver_transection_id=$get_driver_payment_id_prefix.'0000';
					}
////------------------generate new transection id
