...
class DriverDocuments
{
	...
	function document_history($param){
		...
		if(isset($param['driver_eid']) && $param['driver_eid']!=""){
	...
	function driver_documents_quick_totals($param){
		$status=false;
		$message=null;
		$response=null;
		$res=mysqli_fetch_assoc(mysqli_query($GLOBALS['con'],"SELECT 
			(SELECT COUNT(`driver_id`) FROM `drivers` join `documents_types` ON `documents_types`.`document_type_relates_to`='DRIVER' AND `driver_status`='ACT' LEFT JOIN `employee_status` ON `employee_status`.`status_id`=`drivers`.`driver_status_id_fk` WHERE `driver_status`='ACT' AND `status_type_id_fk`='ACTIVE') as `total`,
			(SELECT COUNT(`document_type_id`) FROM `drivers` join `documents_types` ON `document_type_required_option`='T' LEFT JOIN `driver_documents` ON `documents_types`.`document_type_id`=`driver_documents`.`document_type_id_fk` AND `document_driver_id_fk`=`driver_id` LEFT JOIN `employee_status` ON `employee_status`.`status_id`=`drivers`.`driver_status_id_fk` WHERE `driver_status`='ACT' AND `status_type_id_fk`='ACTIVE' AND `document_type_relates_to`='DRIVER' AND NOT `driver_id`=0 AND `document_id` IS NULL) AS `pending_uploads`,
			(SELECT COUNT(`document_type_id`) FROM `drivers` join `documents_types` LEFT JOIN `driver_documents` ON `documents_types`.`document_type_id`=`driver_documents`.`document_type_id_fk` AND `document_driver_id_fk`=`driver_id` LEFT JOIN `employee_status` ON `employee_status`.`status_id`=`drivers`.`driver_status_id_fk` WHERE `driver_status`='ACT' AND `status_type_id_fk`='ACTIVE' AND `document_type_relates_to`='DRIVER' AND NOT `driver_id`=0 AND `document_verification_status`='PENDING') AS `pending_verification`,
			(SELECT COUNT(`document_type_id`) FROM `drivers` join `documents_types` LEFT JOIN `driver_documents` ON `documents_types`.`document_type_id`=`driver_documents`.`document_type_id_fk` AND `document_driver_id_fk`=`driver_id` LEFT JOIN `employee_status` ON `employee_status`.`status_id`=`drivers`.`driver_status_id_fk` WHERE `driver_status`='ACT' AND `status_type_id_fk`='ACTIVE' AND `document_type_relates_to`='DRIVER' AND NOT `driver_id`=0 AND `document_verification_status`='REJECTED') AS `rejected`,
			(SELECT COUNT(`document_type_id`) FROM `drivers` join `documents_types` LEFT JOIN `driver_documents` ON `documents_types`.`document_type_id`=`driver_documents`.`document_type_id_fk` AND `document_driver_id_fk`=`driver_id` LEFT JOIN `employee_status` ON `employee_status`.`status_id`=`drivers`.`driver_status_id_fk` WHERE `driver_status`='ACT' AND `status_type_id_fk`='ACTIVE' AND `document_type_relates_to`='DRIVER' AND NOT `driver_id`=0 AND `document_type_expiry_option`='T' AND DATEDIFF(`document_expriry_date`,CURDATE()) BETWEEN 0 AND `document_type_expiry_alert`) AS `expiry_alert`,
			(SELECT COUNT(`document_type_id`) FROM `drivers` join `documents_types` LEFT JOIN `driver_documents` ON `documents_types`.`document_type_id`=`driver_documents`.`document_type_id_fk` AND `document_driver_id_fk`=`driver_id` LEFT JOIN `employee_status` ON `employee_status`.`status_id`=`drivers`.`driver_status_id_fk` WHERE `driver_status`='ACT' AND `status_type_id_fk`='ACTIVE' AND `document_type_relates_to`='DRIVER' AND NOT `driver_id`=0 AND `document_type_expiry_option`='T' AND DATEDIFF(`document_expriry_date`,CURDATE()) <=0) AS `expired`"));
	...
	function driver_documents_reject($param){
		$status=false;
		$message=null;
		$response=null;
		...
		if(isset($param['document_eid'])){
			...
			$validate_document_id=mysqli_query($GLOBALS['con'],"SELECT `document_id`,`document_type_relates_to` FROM `driver_documents` LEFT JOIN `documents_types` ON `documents_types`.`document_type_id`=`driver_documents`.`document_type_id_fk` WHERE `document_id`='$document_id' AND `document_verification_status`='PENDING'");
		...
		if($dataValidation){
			...
			$verify=mysqli_query($GLOBALS['con'],"UPDATE `driver_documents` SET `document_verification_status`='REJECTED',`document_rejected_on`='$time',`document_rejected_by`='".USER_ID."' WHERE `document_id`='$document_id'");
	...
	function driver_documents_verify($param){
		$status=false;
		$message=null;
		$response=null;
		...
		if($dataValidation){
			...
			$verify=mysqli_query($GLOBALS['con'],"UPDATE `driver_documents` SET `document_verification_status`='VERIFIED',`document_verified_on`='$time',`document_verified_by`='".USER_ID."' WHERE `document_id`='$document_id'");
...
//----verify document details to be uploaded
		...
		if($dataValidation){
			...
			$get_last_id=mysqli_query($GLOBALS['con'],"SELECT `document_id` FROM `driver_documents` ORDER BY `auto` DESC LIMIT 1");
			...
			$create_history_of_old_docs=mysqli_query($GLOBALS['con'],"INSERT INTO `history_driver_documents` (`document_id`, `document_type_id_fk`, `document_file_name`, `document_expriry_date`, `document_driver_id_fk`, `document_truck_id_fk`, `document_status`,`document_remarks`, `document_verification_status`, `document_verified_on`, `document_verified_by`, `document_rejected_on`, `document_rejected_by`, `document_added_on`, `document_added_by`, `document_deleted_on`, `docuemnt_deleted_by`) SELECT `document_id`, `document_type_id_fk`, `document_file_name`, `document_expriry_date`, `document_driver_id_fk`, `document_truck_id_fk`, `document_status`,`document_remarks`, `document_verification_status`, `document_verified_on`, `document_verified_by`, `document_rejected_on`, `document_rejected_by`, `document_added_on`, `document_added_by`, `document_deleted_on`, `docuemnt_deleted_by` FROM `driver_documents` WHERE `document_driver_id_fk`='$driver_id' AND `document_type_id_fk`='$document_type_id'");
			...
			$document_insert=mysqli_query($GLOBALS['con'],"INSERT INTO `driver_documents`(`document_id`, `document_type_id_fk`, `document_file_name`, `document_expriry_date`, `document_driver_id_fk`, `document_status`,`document_remarks`,`document_verification_status`, `document_added_on`, `document_added_by`) VALUES ('$next_id','$document_type_id','$new_file_name','$expiry_date','$driver_id','ACT','$remarks','PENDING','$time','".USER_ID."')");
			if(!$document_insert){
			...
			$delete_old_uploads=mysqli_query($GLOBALS['con'],"DELETE FROM `driver_documents` WHERE `document_type_id_fk`='$document_type_id' AND `document_driver_id_fk`='$driver_id' AND NOT `document_id`='$next_id'");
	...
	function driver_documents($param){
		...
		ValidationChecker:
		if($dataValidation){
			$qEx=mysqli_query($GLOBALS['con'],"SELECT `document_type_id`,`document_type_name`,`document_type_expiry_option`,`document_type_expiry_alert`, `document_type_required_option` ,`document_id`, `document_file_name`, `document_expriry_date`,DATEDIFF(`document_expriry_date`,CURDATE()) AS `expiry_days_left`, `document_added_on`, `document_added_by`, `document_verification_status`, `document_verified_on`, `document_verified_by`,`document_remarks` FROM  `documents_types` LEFT JOIN `driver_documents` ON `documents_types`.`document_type_id`=`driver_documents`.`document_type_id_fk`  AND  `document_driver_id_fk`='$driver_id' WHERE `document_type_relates_to`='DRIVER' ORDER BY `document_type_name`");
...
//-----------apply verification filter
			...
			function all_drivers_documents_list($param){
				...
				ValidationChecker:
				if($dataValidation){
					$q="SELECT `driver_id`,`driver_code`,`driver_name_first`,`document_type_id`,`document_type_name`,`document_type_expiry_option`,`document_type_expiry_alert`, `document_type_required_option` ,`document_id`, `document_file_name`, `document_expriry_date`,DATEDIFF(`document_expriry_date`,CURDATE()) AS `expiry_days_left`, `document_added_on`, `document_added_by`, `document_verification_status`, `document_verified_on`, `document_verified_by`,`document_remarks` FROM `drivers` join `documents_types` LEFT JOIN `driver_documents` ON `documents_types`.`document_type_id`=`driver_documents`.`document_type_id_fk` AND `document_driver_id_fk`=`driver_id` LEFT JOIN `employee_status` ON `employee_status`.`status_id`=`drivers`.`driver_status_id_fk` WHERE `driver_status`='ACT' AND `status_type_id_fk`='ACTIVE' AND `document_type_relates_to`='DRIVER' AND NOT `driver_id`=0 ";
...
//----------/apply filter
					...
					$q .="  ORDER BY `driver_code`,`document_type_name` limit $from, $batch";
					...
					while ($rows=mysqli_fetch_assoc($qEx)) {
						...
						array_push($list,[													'sr_no'=>++$counter,
							'driver_id'=>$rows['driver_id'],
							'driver_eid'=>$Enc->safeurlen($rows['driver_id']),
